...
- [**agent loop 逐段精读**](/pi/agent-loop) —— 整个项目最该读的 792 行：双层循环、工具调度的两种顺序、以及那些踩过坑才会写出来的防御。
- [**pi-ai 逐段精读**](/pi/pi-ai) —— 46 个 provider 与 10 种协议如何解耦：加一家厂商 15 行、加一种协议一千行；以及 24 条正则只为判断「上下文超了」。

两篇有三处明确的接缝：流契约（谁保证不抛异常）、不完整的工具参数（谁负责解析、谁负责不信任）、上下文压力（谁负责识别、谁负责处理）。建议按顺序读。

## 待写

- coding-agent 的工具与扩展系统：`Operations` 接口如何让工具重定向到沙箱
